...
class CardTypeSeriesCustom extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      onTap: () {context.read<CardTypeSeriesBloc>().add(CardTypeSeriesPressed(text: text));},
      child: BlocBuilder<CardTypeSeriesBloc, CardTypeSeriesState>(
        builder: (context, state) {
          bool isSelected  = isSelectedCaseBool(state);
          return Opacity(
  ...
  bool isSelectedCaseBool(CardTypeSeriesState state) {
    return state is CardTypeSeriesSelected && state.numberSelected == text ? true : false;
  }
}
